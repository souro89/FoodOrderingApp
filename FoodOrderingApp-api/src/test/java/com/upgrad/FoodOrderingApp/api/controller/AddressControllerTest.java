...
public class AddressControllerTest {
    ...
    public void shouldGetAllAddresses() throws Exception {
        ...
        addressEntity.setFaltBuilNumber("flatBuildNo");
        final String stateUuid = UUID.randomUUID().toString();
        addressEntity.setStateEntity(new StateEntity(stateUuid, "state"));
        when(mockAddressService.getAddresses(customerEntity)).thenReturn(Collections.singletonList(addressEntity));
        ...
        verify(mockCustomerService, times(1)).getCustomer("database_accesstoken2");
        verify(mockAddressService, times(1)).getAddresses(customerEntity);
    }
    ...
    public void shouldNotGetAllAddressesWithNonExistingAccessToken() throws Exception {
        ...
        mockMvc
                ...
                .andExpect(jsonPath("code").value("ATHR-001"));
        verify(mockCustomerService, times(1)).getCustomer("non_existing_access_token");
        verify(mockAddressService, times(0)).getAddresses(any());
    }
    ...
    public void shouldGetAllStates() throws Exception {
        ...
        when(mockAddressService.getStates()).thenReturn(Collections.singletonList(stateEntity));
    ...
    public void shouldNotGetAllStatesIfNonePresentInDb() throws Exception {
        when(mockAddressService.getStates()).thenReturn(Collections.emptyList());
